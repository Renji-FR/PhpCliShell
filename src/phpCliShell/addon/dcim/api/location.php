	...
	class Location extends AbstractApi
	{
		...
		public function getParentLocationApi()
		{
			if($this->_parentLocationApi === null)
			{
				...
				if($locationId !== false) {
					$this->_parentLocationApi = Location::factory($locationId);
				}
				else {
		...
		public function findCabinets($cabinetLabel, $recursion = false)
		{
			if($this->hasLocationId()) {
				$locationId = $this->getLocationId();
				return Cabinet::searchCabinets($cabinetLabel, $locationId, $recursion);
			}
			else {
				return false;
			}
		...
		public function findEquipments($equipmentLabel, $equipmentDesc, $equipmentSN, $recursion = false)
		{
			if($this->hasLocationId()) {
				$locationId = $this->getLocationId();
				return Equipment::searchEquipments($equipmentLabel, $equipmentDesc, $equipmentSN, null, $locationId, $recursion);
			}
			else {
				return false;
			}
