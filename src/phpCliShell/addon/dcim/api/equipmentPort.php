<?php
	...
	class EquipmentPort extends AbstractInterface
	{
		...
		/**
		  ...
		  *
		  * @return false|\PhpCliShell\Addon\Dcim\Api\Equipment
		  */
		public function getEquipmentApi()
		{
			if($this->_equipmentApi === null)
			{
				...
				if($equipmentId !== false) {
					$this->_equipmentApi = Equipment::factory($equipmentId);
				}
				else {
		...
		public function getCableApi()
		{
			if($this->_cableApi === null)
			{
				...
				if($cableId !== false) {
					$this->_cableApi = Cable::factory($cableId);
				}
				else {
