<?php
	...
	class Cabinet extends AbstractLocality
	{
			...
			if($this->cabinetExists())
			{
				if($this->_locationId === null)
				{
					...
					if(C\Tools::is('array&&count>0', $equipmentIds))
					{
						...
						$Api_Equipment = Equipment::factory($equipmentId);
		...
		public function getEquipmentIdByU($positionU)
		{
			...
			{
				$equipments = Equipment::searchEquipments('*', null, null, $this->getCabinetId(), null, false, true);
		...
		public function findEquipments($equipmentLabel, $equipmentDesc, $equipmentSN, $recursion = false, $firstLevel = true)
		{
			if($this->hasCabinetId()) {
				...
				return Equipment::searchEquipments($equipmentLabel, $equipmentDesc, $equipmentSN, $cabinetId, null, $recursion, $firstLevel);
			}
			else {
				return false;
			}
