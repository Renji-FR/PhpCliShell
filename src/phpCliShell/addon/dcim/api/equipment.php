<?php
	...
	class Equipment extends AbstractLocality
	{
		...
		/**
		  * @var \PhpCliShell\Addon\Dcim\Api\Cabinet
		  */
		...
		public function getCabinetApi()
		{
			if($this->_cabinetApi === null)
			{
				...
				if($cabinetId !== false) {
					$this->_cabinetApi = Cabinet::factory($cabinetId);
				}
				else {
		...
		public function rack($cabinetName, $side, $positionU, C\Item $template, $label = null, $description = null, $positionX = 0, $autoRegisterToStore = true)
		{
			...
			if(!$this->equipmentExists())
			{
				if($this->hasLocationId())
				{
					...
					{
						if($side === Cabinet::SIDE_FRONT || $side === Cabinet::SIDE_REAR)
						{
