<?php
	...
	{
		const PORT_INTERFACE_SEPARATOR = Port::INTERFACE_SEPARATOR;
		const INT_INTERFACE_SEPARATOR = Interface_::INTERFACE_SEPARATOR;
		...
		/**
		  * @var \PhpCliShell\Addon\Dcim\Equipment\Module[]
		  */
		...
		/**
		  ...
		  * @param null|\PhpCliShell\Addon\Dcim\Api\Equipment $apiEquipment
		  * @return $this
		  */
		public function __construct($equipmentId, Api\Equipment $apiEquipment = null)
		{
			...
			if($apiEquipment !== null && $apiEquipment->id === $this->_equipmentId) {
				...
				$this->_equipmentApi = new Api\Equipment($this->_equipmentId);
			}
		...
		protected function _retrieveModuleId($attribute)
		{
			...
			elseif($attribute instanceof Port) {
				$moduleId = $attribute->getModuleId();
			}
			elseif($attribute instanceof Interface_)
			{
				if($attribute->isPhysical()) {
					...
					throw new Exception("It is not possible to retrieve module from logical interface '".$attribute->interfaceName."'", E_USER_ERROR);
				}
			}
			elseif($attribute instanceof Module) {
				$moduleId = $attribute->getModuleId();
			}
		...
		/**
		  * @param mixed $attribute
		  * @return \PhpCliShell\Addon\Dcim\Equipment\Module
		  */
		protected function _retrieveModule($attribute)
		{
			...
			if(!array_key_exists($moduleId, $this->_modules)) {
				$this->_modules[$moduleId] = new Module($this, $moduleId);
			}
		...
		public function retrieveSlot($name)
		{
			if(is_object($name))
			{
				if($name instanceof Slot) {
					...
					throw new Exception("Slot name must be a string or an Dcim\Equipment\Slot object, '".gettype($name)."' given", E_USER_ERROR);
				}
			}
			else
			{
		...
		public function retrievePort($name)
		{
			if(is_object($name))
			{
				if($name instanceof Port) {
					...
					throw new Exception("Port name must be a string or an Dcim\Equipment\Port object, '".gettype($name)."' given", E_USER_ERROR);
				}
			}
			else
			{
		...
		/**
		  * @param \PhpCliShell\Addon\Dcim\Equipment\Port $equipmentPort
		  * @return void
		  */
		protected function _newInterface(Port $equipmentPort)
		{
			$Equipment_Interface = new Interface_($equipmentPort);
		...
		public function retrieveInterface($name, $index = null)
		{
			if(is_object($name))
			{
				if($name instanceof Interface_)
				{
					...
					throw new Exception("Interface name must be a string or an Dcim\Equipment\Interface_ object, '".gettype($name)."' given", E_USER_ERROR);
				}
			}
			else
			{
		...
		/**
		  * @param \PhpCliShell\Addon\Dcim\Api\Equipment $equipmentApi
		  * @return string Equipment label
		  */
		protected static function _formatEquipmentLabel(Api\Equipment $equipmentApi)
		{
			$equipmentLabel = $equipmentApi->getEquipmentLabel();

			if($equipmentLabel === false) {
				$equipmentId = $equipmentApi->getEquipmentId();
		...
		/**
		  * @param bool|int Debug status or level
		  * @return bool|int Debug status or level
		  */
