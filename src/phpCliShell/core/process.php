<?php
	...
	class Process
	{
		...
		public function exec($command, $enter = "\n")
		{
			if($this->isRunning()) {
				$this->_stdout = $this->_stderr = "";
				$status = fwrite($this->_pipes[self::STDIN], $command.$enter);
				return ($status !== false);
			}
			else {
				return false;
		...
		/**
		  ...
		  * @param int $timeout
		  * @param false|string $stopOnStdoutRegex
		  * @param false|string $stopOnStderrRegex
		  * @return mixed
		  */
		public function flushPipes(Closure $callback = null, $timeout = self::TIMEOUT_PIPES, $stopOnStdoutRegex = false, $stopOnStderrRegex = false)
		{
			$isTimeout = $this->_waitingPipes(null, 0, $timeout, false, $stopOnStdoutRegex, $stopOnStderrRegex);

			if($callback !== null) {
				return $callback($this, $this->_stdout, $this->_stderr);
			}
			else {
				return array(self::STDOUT => $this->_stdout, self::STDERR => $this->_stderr);
			}
		}
		...
		/**
		  * @param null|\Closure $callback
		  * @param int $timeout
		  * @param bool $stopOnOutput
		  * @param false|string $stopOnStdoutRegex
		  * @param false|string $stopOnStderrRegex
		  * @return mixed
		  */
		public function waitingPipes(Closure $callback = null, $timeout = self::TIMEOUT_PIPES, $stopOnOutput = true, $stopOnStdoutRegex = false, $stopOnStderrRegex = false)
		{
			$isTimeout = $this->_waitingPipes(null, 0, $timeout, $stopOnOutput, $stopOnStdoutRegex, $stopOnStderrRegex);

			if($isTimeout) {
				throw new Exception("Process idle timeout (".$timeout."s) expired", E_USER_ERROR);
			}
			else
			{
				if($callback !== null) {
		...
		/**
		  ...
		  * @return mixed
		  */
		public function waitingCustomPipes(Closure $callback = null, $timeRate = 2, $timeout = self::TIMEOUT_PIPES)
		{
			$isTimeout = $this->_waitingPipes($callback, $timeRate, $timeout, false, false, false);
		...
		/**
		  * Lors d'un process sans interraction on attend:
		  * - que les pipes se ferment
		  * - un match sur stdout/stderr
		  * - au premier output stdout/stderr
		  *
		  * @param null|\Closure $callback
		  * @param int $timeRate
		  * @param int $timeout
		  ...
		  */
		protected function _waitingPipes(Closure $callback = null, $timeRate = 2, $timeout = self::TIMEOUT_PIPES, $stopOnOutput = true, $stopOnStdoutRegex = false, $stopOnStderrRegex = false)
		{
			...
			do
			{
				...
				if($stopOnStdoutRegex !== false || $stopOnStderrRegex !== false)
				{
					$continue = (
						($stopOnStdoutRegex === false || !preg_match($stopOnStdoutRegex, $this->_stdout)) &&
						($stopOnStderrRegex === false || !preg_match($stopOnStderrRegex, $this->_stderr))
					);
				}
				elseif($stopOnOutput) {
					$continue = !$hasOutput;
				...
				elseif($callback !== null) {
					...
					$continue = $callback($this, $this->_lastOutput, $this->_lastError);
				}
				else {
					$continue = true;
				}
				...
				var_dump($status, $hasOutput, $isTimeout, $continue, $this->_lastOutput, $this->_lastError);*/
			}
			while($this->isRunning() && $status && !$isTimeout && $continue);
		...
		public function waitingCustomClose(Closure $callback = null, $timeRate = 2, $timeout = self::TIMEOUT_CLOSE)
		{
			...
			do
			{
				...
				if($callback !== null) {
					$usTime = microtime(true) - $startTime;
					usleep(($timeRate * 1000000) - $usTime);
					$callback($this, $this->_lastOutput, $this->_lastError);
				}
			}
		...
		public function waitingClose($timeout = self::TIMEOUT_CLOSE)
		{
			...
			return $this->waitingCustomClose(null, 0, $timeout);
		}
		...
		protected function _getDataPipes($blocking = true)
		{
			...
			$this->_lastOutput = $this->_lastError = "";
			...
			if(count($readPipes) > 0)
			{
				...
				try {
					// stream_select modifie readPipes avec les pipes à traiter
