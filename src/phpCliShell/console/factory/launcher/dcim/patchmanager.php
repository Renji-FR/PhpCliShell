<?php
	namespace PhpCliShell\Console\Factory\Launcher\Dcim;
	...
	class Patchmanager extends AbstractLauncher
	{
		const TEMPLATE_PATHNAME = __DIR__ .'/templates';
		const TEMPLATE_FILENAME = 'patchmanager.php';

		const LAUNCHER_PATHNAME = self::DEFAULT_LAUNCHER_PATHNAME;
		const LAUNCHER_FILENAME = 'patchmanager.%s.php';

		const CONFIG_SECTION_ROOT_NAME = Configuration\Dcim\Patchmanager::CONFIG_SECTION_ROOT_NAME;
		...
		public function factory()
		{
			...
			// server key
			// --------------------------------------------------
			$question = new Question("What is the key name in configuration of your PatchManager service? (Key must be unique. Only [A-Z0-9_-] characters are allowed) ", false, '#[0-9A-Z_\-]+#i');
			...
			// Build launcher
			// --------------------------------------------------
			$configPathname = Configuration\Dcim\Patchmanager::retrieveConfigurationPathname($serverKey);
