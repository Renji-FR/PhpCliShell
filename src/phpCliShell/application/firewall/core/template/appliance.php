<?php
	...
	abstract class Appliance extends Hardware
	{
		...
		/**
		  * @param \PhpCliShell\Application\Firewall\Core\Api\Site[] $sites
		  * @param \PhpCliShell\Core\ArrayObject $filters
		  * @param \PhpCliShell\Application\Firewall\Core\Api\AclRule $ruleApi
		  * @return bool
		  */
		protected function _ruleProcessing(array $sites, C\ArrayObject $filters, Api\AclRule $ruleApi)
		{
			...
			foreach(array('src' => 'sources', 'dst' => 'destinations') as $attr => $attributes)
			{
				...
				foreach($ruleApi->{$attributes} as $Api_Address)
				{
					...
					{
						if(!isset($addressZones[$Api_Address::API_TYPE][$Api_Address->id])) {
							$zone = $this->_execFilter($filters[$topology], $Api_Address);
						}
						else {
							$zone = $addressZones[$Api_Address::API_TYPE][$Api_Address->id];
						}
						...
						if($zone !== false)
						{
							if($action)
							{
								...
								if(!isset($addressZones[$Api_Address::API_TYPE][$Api_Address->id])) {
									$addressZones[$Api_Address::API_TYPE][$Api_Address->id] = $zone;
									$addressTopologies[$Api_Address::API_TYPE][$Api_Address->id] = $topology;
								}
		...
		/**
		  ...
		  * @throw Exception
		  */
		protected function _createFailoverRules(array $sites, Api\AclRule $ruleApi)
		{
			...
			if($policyAcl === false) {
				throw new Exception("Unable to retrieve original ACL rule '".$ruleApi->name."'", E_USER_ERROR);
			}
		...
		/**
		  * @param \PhpCliShell\Application\Firewall\Core\Api\Site $siteApi
		  * @param \PhpCliShell\Application\Firewall\Core\Api\Address $srcAttribute
		  * @param \PhpCliShell\Application\Firewall\Core\Api\Address $dstAttribute
		  ...
		  * @throw Exception
		  */
		protected function _getFailoverZone(Api\Site $siteApi, Api\Address $srcAttribute, Api\Address $dstAttribute, &$srcZone, &$dstZone, &$useGlobalZone, &$addressZones)
		{
			$zones = $this->_siteApi->zones;
			$topology = $this->_siteApi->topology;
			...
			if($srcZoneMatching !== false && $dstZoneMatching !== false)
			{
				...
				foreach(array('src' => $srcZoneMatching, 'dst' => $dstZoneMatching) as $attr => $attrZoneMatching)
				{
					...
					if(!isset($addressZones[${$attr.'Attribute'}::API_TYPE][${$attr.'Attribute'}->id])) {
						$addressZones[${$attr.'Attribute'}::API_TYPE][${$attr.'Attribute'}->id] = $attrZoneMatching;
					}
		...
		/**
		  ...
		  */
		protected function _getFullmeshAttrs(C\ArrayObject $filters, array $attributes)
		{
			$theZone = null;
			$results = array();
		...
		/**
		  ...
		  * @return \PhpCliShell\Core\ArrayObject Filters
		  */
		...
		/**
		  ...
		  * @return \PhpCliShell\Core\ArrayObject Filters
		  */
		protected function _getFormatedFilters(C\ArrayObject $filters, $IPv)
		{
			$results = new C\ArrayObject();
			...
			foreach($filters as $name => $filter)
			{
				...
				elseif($filter instanceof C\ArrayObject) {
		...
		/**
		  ...
		  * @throw Exception
		  */
		protected function _execFilter(C\ArrayObject $filters, Api\Address $addressApi)
		{
		...
		/**
		  ...
		  */
		protected function _isMatchingFilters($method, C\ArrayObject $filters, $attribute, &$maskMatch = false)
		{
			$nameMatch = false;

			foreach($filters as $name => $filter)
			{
				if($filter instanceof C\ArrayObject)
				{
				...
				{
					$callable = array(Core\Tools::class, $method);
		...
		/**
		  ...
		  * @return array|false 0:Api\Site, 1:zone matching
		  */
		...
		/**
		  ...
		  */
		protected function _prepareToObjectAdd(Api\Address $addressApi, $zone = null)
		{
		...
		/**
		  ...
		  * @return array|\ArrayObject Object address datas
		  */
		abstract protected function _toObjectAdd(Api\Address $addressApi, $zone = null);
		...
		/**
		  ...
		  */
		protected function _getProtocolApp(Api\Protocol $protocolApi, $zone = null)
		{
		...
		/**
		  ...
		  */
		protected function _prepareToProtocolApp(Api\Protocol $protocolApi, $zone = null)
		{
		...
		/**
		  ...
		  * @return array|\ArrayObject Protocol application datas
		  */
		abstract protected function _toProtocolApp(Api\Protocol $protocolApi, $zone = null);
		...
		/**
		  ...
		  */
		protected function _getPolicyAcl(Api\AclRule $ruleApi, $zone = null)
		{
		...
		/**
		  ...
		  */
		protected function _prepareToPolicyAcl(Api\AclRule $ruleApi, array $srcZones, array $sources, array $dstZones, array $destinations)
		{
		...
		/**
		  ...
		  * @return array|\ArrayObject Policy accesslist datas
		  */
		abstract protected function _toPolicyAcl(Api\AclRule $ruleApi, array $srcZones, array $sources, array $dstZones, array $destinations);
	}