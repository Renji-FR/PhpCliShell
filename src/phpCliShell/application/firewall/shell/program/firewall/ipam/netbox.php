<?php
	...
	class Netbox extends AbstractIpam
	{
		/**
		  * @return false|\PhpCliShell\Addon\Ipam\Netbox\Orchestrator
		  */
		protected function _initAddonOrchestrator()
		{
			if($this->isAddonPresent())
			{
				if(!AddonIpam\Orchestrator::hasInstance()) {
					$orchestrator = AddonIpam\Orchestrator::getInstance($this->_CONFIG->IPAM_NETBOX);
		...
		public function searchSubnets($search, $strict = false)
		{
			...
			if($orchestrator !== false && count($orchestrator) > 0)
			{
				...
				foreach($orchestrator as $Ipam_Service)
				{
					...
					$cidrSubnets = AddonIpam\Api\Subnets::searchCidrSubnets($search, null, null, $strict);
					...
					/**
					  ...
					  */
					$subnetNames = AddonIpam\Api\Subnets::searchSubnetNames($search, null, null, null, $strict);
					...
					if(C\Tools::is('array&&count>0', $cidrSubnets))
					{
						...
						foreach($cidrSubnets as $index => $cidrSubnet)
						{
							...
							{
								$_subnetNames = AddonIpam\Api\Subnets::searchSubnetNames($cidrSubnet[AddonIpam\Api\Subnet::FIELD_NAME], null, null, null, $strict);
