<?php
	...
	class Address extends AbstractManager
	{
		/**
		  * @var string
		  */
		const MANAGER_TYPE = 'addressManager';

		/**
		  * @var string
		  */
		const MANAGER_LABEL = 'address';
		...
		/**
		  * @var false|\PhpCliShell\Application\Firewall\Shell\Program\Firewall\Ipam
		  */
		protected $_ipamFwProgram = null;


		/**
		  * @return \PhpCliShell\Application\Firewall\Shell\Program\Firewall\Renderer\Address
		  */
		public function getRenderer()
		{
			return new Renderer\Address();
		}

		/**
		  * @return false|\PhpCliShell\Application\Firewall\Shell\Program\Firewall\Ipam
		  */
		protected function _getIpamFwProgram()
		{
			if($this->_ipamFwProgram === null) {
				$this->_ipamFwProgram = $this->_firewallProgram->getProgram('ipam');
			}

			return $this->_ipamFwProgram;
		}
		...
		public function create($type, array $args)
		{
			if($this->_typeIsAllowed($type))
			{
				if(isset($args[0]) && isset($args[1]))
				{
					...
					if($Core_Api_Address instanceof Api\Address)
					{
						$objectName = ucfirst($Core_Api_Address::API_LABEL);
		...
		public function modify($type, array $args)
		{
			if(isset($args[0]) && isset($args[1]))
			{
				...
				if(($Core_Api_Address = $this->getObject($type, $name)) !== false)
				{
					...
					if($status)
					{
						$rules = $this->_objects[Api\AclRule::API_TYPE];
						...
						foreach($rules as $Core_Api_Rule)
						{
							...
							if($isPresent)
							{
								...
								if(($Core_Api_Address__like = $Core_Api_Rule->getAddressIsInUse($Core_Api_Address, true, $Core_Api_Address)) !== false) {
									$message = $Core_Api_Address__like::API_LABEL." '".$Core_Api_Address__like->name."' possède un adressage identique";
				...
				if(($Core_Api_Address = $this->getObject($type, $name)) !== false)
				{
					...
					foreach($rules as $Core_Api_Rule)
					{
						if($Core_Api_Rule->isPresent($Core_Api_Address)) {
							$this->_SHELL->error("La règle ACL '".$Core_Api_Rule->name."' utilise cet object", 'orange');
							$isUsed = true;
							break;
						}
					}
			...
			if($this->_typeIsAllowed($type))
			{
				...
				if($Core_Api_Address !== false)
				{
					...
					if(count($results) > 0) {
						...
						$this->_SHELL->print("Aucune règle ACL ne semble correspondre à cet objet ".$objectName, 'green');
					}
				}
				else {
		...
		protected function _refresh($type, Api\Address $addressApi)
		{
			$name = $addressApi->name;
			$Core_Api_Address__clone = clone $addressApi;
			$objectName = ucfirst($addressApi::API_LABEL);
								...
								foreach($rules as $Core_Api_Rule)
								{
									...
									if($isPresent)
									{
										...
										if(($Core_Api_Address = $Core_Api_Rule->getAddressIsInUse($addressApi, true, $addressApi)) !== false) {
											$message = $Core_Api_Address::API_LABEL." '".$Core_Api_Address->name."' possède un adressage identique";
											$this->_SHELL->error("RULE '".$Core_Api_Rule->name."': ".$message, 'red');
										}
									}
								}
		...
		/**
		  ...
		  * @return null|\PhpCliShell\Application\Firewall\Core\Api\Address
		  */
		public function getLocalObjectApi($type, $arg, $strictKey = true)
		{
			/**
			  * En mode strict, il ne faut pas que la recherche puisse retourner plusieurs résultats
			  * En mode non strict, on sélectionne le premier résultat trouvé puisqu'il correspond
			  */
			$objects = $this->getObjects($type, $arg, $strictKey, true, 1);
		...
		public function getIpamObjectApi($type, $arg, $strict = true)
		{
			...
			$ipamFwProgram = $this->_getIpamFwProgram();

			if($ipamFwProgram !== false)
			{
				$results = $ipamFwProgram->getObjects($type, $arg, $strict);		// Pas de try/catch afin de laisser le parent gérer de lui-même
				$class = $this->_typeToClass($type);
		...
		/**
		  ...
		  * @param string $arg Object name or address
		  * @param bool $strictKey Strict key search mode
		  * @param bool $allowCreateFromAddress Allow to create object from address or not
		  * @return null|false|\PhpCliShell\Application\Firewall\Core\Api\Address Return null if object can not be found, false if error occur or the object API instance found
		  */
		public function autoCreateObject($type, $arg, $strictKey = false, $allowCreateFromAddress = true)
		{
			if($this->_typeIsAllowed($type))
			{
				...
				if($Core_Api_Address === null)
				{
					...
					elseif($Core_Api_Address instanceof Api\Address)
					{
						/**
						...
						if($Core_Api_Address instanceof Api\Subnet) {
							$state = $this->_register($Core_Api_Address, false, true);
						}
						elseif($Core_Api_Address instanceof Api\Host) {
							$state = $this->_register($Core_Api_Address, false, true);
						}
						else {
		...
		/**
		  * @param string|\PhpCliShell\Application\Firewall\Core\Api\Address $address
		...
		public function getAutoNamingAddress($address)
		{
			/**
			  ...
			  */
			if($address instanceof Api\Address) {
				$address = $address->ipv4.'_'.$address->ipv6;
				$address = trim($address, '_');
				$isValid = true;
			}
			elseif(Core\Tools::isIP($address) || Core\Tools::isSubnet($address) || Core\Tools::isNetwork($address, Api\Network::SEPARATOR)) {
				$isValid = true;
			}
			else {
				$isValid = false;
