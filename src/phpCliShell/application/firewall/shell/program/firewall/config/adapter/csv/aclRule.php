<?php
	...
	class AclRule extends AbstractRule
	{
		...
		/**
		  ...
		  */
		public function importRules(array $aclRules, $keepName, $prefix, $suffix, $checkValidity)
		{
			$csvName = null;
			$ruleCounter = 0;
			...
			{
				if(count($rule) !== 11) {
					throw new E\Message("Impossible d'importer les règles ACL, fichier CSV non valide", E_USER_ERROR);
				}
				...
				switch($rule[1])
				{
					case Api\AclRule::CATEGORY_FAILOVER: {
						$category = Api\AclRule::CATEGORY_FAILOVER;
						break;
					}
					default: {
						$category = Api\AclRule::CATEGORY_MONOSITE;
					}
				}

				$fullmesh = ($rule[2] === 'fullmesh');
				$state = ($rule[3] === 'active');
				$action = ($rule[4] === 'permit');
				...
				if($Core_Api_Address__src instanceof Api\Address && $Core_Api_Address__dst instanceof Api\Address)
				{
					...
					if($isValidProtocol && $Core_Api_Protocol->isValid())
					{
						...
						if(!$append)
						{
							...
							try {
								$Core_Api_Rule = $this->_aclRuleFwProgram->insert($type, $name, null);
							}
							catch(\Exception $e) {
								$this->_SHELL->throw($e);
								$Core_Api_Rule = null;
							...
							if($Core_Api_Rule instanceof Api\AclRule)
							{
						...
						{
							/**
							  * /!\ Risque de doublon si Api\AclRule n'a pas la sécurité
							  *
							  ...
							  *
							  * @todo coder une vérification en amont ou laisser Api\AclRule faire le job de vérification?
							  */
							...
							{
								try {
									$status = $this->_aclRuleFwProgram->drop($Core_Api_Rule);
								}
								catch(\Exception $e) {
									$this->_SHELL->throw($e);
									$status = null;
		...
		public function exportRules(array $configs)
		{
			...
			if(array_key_exists(Api\AclRule::API_TYPE, $configs))
			{
				$rules = $configs[Api\AclRule::API_TYPE];
				...
				foreach($rules as $rule)
				{
					...
					$item[9] = implode(static::TAG_SEPARATOR, $rule['tags']);
					...
					$typeSeparator = preg_quote(Api\AclRule::SEPARATOR_TYPE, '#');
