<?php
	namespace PhpCliShell\Application\Netbox\Shell\Program;
	...
	use PhpCliShell\Application\Netbox\Shell\Exception;
	...
	class Ipam extends Cli\Shell\Program\Browser
	{
		...
		/**
		  * @var \PhpCliShell\Addon\Ipam\Netbox\Service\Store
		  */
		...
		protected function _getSubnetResults($subnet, $fromCurrentPath = true, $path = null)
		{
			...
			if($fromCurrentPath && count($subnets) > 0)
			{
				...
				$currentFolderApi = $this->_getLastFolderPath($pathApi);

				if($currentFolderApi !== false)
				{
					$currentFolderId = $currentFolderApi->getFolderId();
					...
					foreach($subnets as $index => $subnet)
					{
						...
						if($Ipam_Api_Subnet->getFolderId() !== $currentFolderId) {
							unset($subnets[$index]);
						}
				...
				foreach($cases[$currentType] as $objectClass => $objectMethod)
				{
					...
					if(C\Tools::is('array&&count>0', $objects))
					{
						...
						foreach($objects as $object)
						{
							switch($objectType)
							{
								case AddonIpam\Api\Subnet::OBJECT_TYPE:
								{
									$subnetApi = AddonIpam\Api\Subnet::factory($object[$objectClass::FIELD_ID]);
									$subnetName = $this->formatSubnetNameWithIPv($subnetApi->getName(), $subnetApi->getIPv());
					...
					elseif($currentApi instanceof AddonIpam\Api\Subnet)
					{
						...
						if($addresses !== false)
						{
							foreach($addresses as $address)
							{
								$addressState = $address['status_label'];
					...
					if($status)
					{
						...
						if($status)
						{
							...
							if($status)
							{
								...
								try {
									$status = $Ipam_Api_Address->create(AddonIpam\Api\Address::ACTIVE);
								}
		...
		protected function _getLastSectionPath(array $pathApi)
		{
			...
			// /!\ La toute 1ere section n'existe pas, voir PhpCliShell\Application\Netbox\Shell\Ipam::_moveToRoot
		...
		/**
		  * @param \PhpCliShell\Addon\Ipam\Netbox\Api\Subnet $subnetApi
		...
		{
			$cidrSubnet = $subnet[AddonIpam\Api\Subnet::FIELD_SUBNET];
		...
		/**
		  * @param \PhpCliShell\Addon\Ipam\Netbox\Api\InterfaceApi $baseApi
		...
		protected function _shellAutoC_cd_browser($baseApi, $search = null)
		{
			...
			if($baseApiClassName === AddonIpam\Api\Folder::class || $baseApiClassName === AddonIpam\Api\Subnet::class)
			{
				$userSearch = $search;
					...
					if($subnets !== false)
					{
						array_walk($subnets, function (&$subnet)
						{
							if(!C\Tools::is('string&&!empty', $subnet[AddonIpam\Api\Subnet::FIELD_NAME])) {
								$subnet[AddonIpam\Api\Subnet::FIELD_NAME] = $subnet[AddonIpam\Api\Subnet::FIELD_SUBNET];
							}
							else {
