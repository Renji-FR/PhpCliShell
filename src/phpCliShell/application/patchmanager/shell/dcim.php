<?php
	namespace PhpCliShell\Application\Patchmanager\Shell;
	...
	use PhpCliShell\Application\Patchmanager\Shell\Program;

	class Dcim extends Cli\Shell\Browser
	{
		const SHELL_HISTORY_FILENAME = '.dcim_patchmanager.history';
		...
		/**
		  * @var \PhpCliShell\Addon\Dcim\Service
		  */
		...
		public function __construct($configuration, $server, $autoInitialisation = true)
		{
			...
			$this->_PROGRAM = new Program\Dcim($this, $this->_TERMINAL);
		...
		{
			$Dcim_Orchestrator = AddonDcim\Orchestrator::getInstance($this->getAddonConfig());
			...
			if($printInfoMessages) {
				...
				C\Tools::e(PHP_EOL."Connection ".$adapterMethod." au DCIM [".$this->_addonService::SERVICE_NAME."] {".$adapterLabel."} @ ".$server." veuillez patienter ... ", 'blue');
			}
			...
			catch(\Exception $e)
			{
				...
				$this->error("Impossible de démarrer le service DCIM:", 'orange');
		...
		protected function _refreshAddonCaches()
		{
			...
			{
				$classes = array(
					AddonDcim\Api\Location::class,
					AddonDcim\Api\Cabinet::class,
					AddonDcim\Api\Equipment::class,
				);
		...
		{
			return $this->_CONFIG->DCIM_PATCHMANAGER;
		}
		...
		protected function _preLauchingShell($welcomeMessage = true)
		{
			...
			/**
			  ...
			  */
			$rootLocations = AddonDcim\Api\Location::getRootLocations();

			if(count($rootLocations) > 0 && count($rootLocations[0]) === 1) {
				$csvDelimiter = $this->_addonService->config->preferences->report->csvDelimiter;
		...
		protected function _moveToRoot()
		{
			...
			{
				$Dcim_Api_Location = new AddonDcim\Api\Location();
		...
		public function browser(array &$pathIds, array &$pathApi, $path)
		{
			...
			/**
			  ...
			  * Voir PhpCliShell\Application\Phpipam\Shell\Ipam->browser
			  */
			$cases = array(
				AddonDcim\Api\Location::OBJECT_TYPE => array(
					AddonDcim\Api\Location::class => 'getSubLocationId',
					AddonDcim\Api\Cabinet::class => 'getCabinetId',
				),
				AddonDcim\Api\Cabinet::OBJECT_TYPE => array(
					AddonDcim\Api\Equipment::class => 'getEquipmentId',
				),
			);
