<?php
	namespace PhpCliShell\Application\Patchmanager\Shell\Program;

	use PhpCliShell\Core as C;

	use PhpCliShell\Cli as Cli;

	use PhpCliShell\Addon\Dcim as AddonDcim;

	use PhpCliShell\Application\Patchmanager\Shell\Exception;

	class Dcim extends Cli\Shell\Program\Browser
	{
		const OBJECT_NAMES = array(
				AddonDcim\Api\Location::OBJECT_TYPE => AddonDcim\Api\Location::OBJECT_NAME,
				AddonDcim\Api\Cabinet::OBJECT_TYPE => AddonDcim\Api\Cabinet::OBJECT_NAME,
				AddonDcim\Api\Equipment::OBJECT_TYPE => AddonDcim\Api\Equipment::OBJECT_NAME,
				AddonDcim\Api\Cable::OBJECT_TYPE => AddonDcim\Api\Cable::OBJECT_NAME,
		);

		const RESULT_KEYS = array(
				AddonDcim\Api\Location::OBJECT_TYPE => 'locations',
				AddonDcim\Api\Cabinet::OBJECT_TYPE => 'cabinets',
				AddonDcim\Api\Equipment::OBJECT_TYPE => 'equipments',
				AddonDcim\Api\Cable::OBJECT_TYPE => 'cables',
		);
		...
		/**
		  * @var \PhpCliShell\Addon\Dcim\Service\Store
		  */
		...
		public function __construct(Cli\Shell\Main $SHELL)
		{
			...
			$this->_addonStore = AddonDcim\Orchestrator::getInstance()->service->store;
		}
		...
		{
			return $this->_listObjects(AddonDcim\Api\Location::OBJECT_TYPE, $args);
		}

		public function showLocations(array $args)
		{
			return $this->_showObjects(AddonDcim\Api\Location::OBJECT_TYPE, $args);
		}

		public function listCabinets(array $args)
		{
			return $this->_listObjects(AddonDcim\Api\Cabinet::OBJECT_TYPE, $args);
		}

		public function showCabinets(array $args)
		{
			return $this->_showObjects(AddonDcim\Api\Cabinet::OBJECT_TYPE, $args);
		}
		...
		protected function _printCabinetExtra(array $cabinets, array $args = null)
		{
			if(count($cabinets) === 1)
			{
				...
				$this->_RESULTS[self::RESULT_KEYS[AddonDcim\Api\Equipment::OBJECT_TYPE]] = $objects['equipment'];
			}
		}

		public function listEquipments(array $args)
		{
			return $this->_listObjects(AddonDcim\Api\Equipment::OBJECT_TYPE, $args);
		}

		public function showEquipments(array $args)
		{
			return $this->_showObjects(AddonDcim\Api\Equipment::OBJECT_TYPE, $args);
		}
		...
		protected function _printEquipmentExtra(array $equipments, array $args = null)
		{
			if(count($equipments) === 1)
			{
				...
				$this->_RESULTS[self::RESULT_KEYS[AddonDcim\Api\Cable::OBJECT_TYPE]] = $objects['cable'];
			}
		}
			...
			if(isset($args[0]))
			{
				...
				if(count($items) > 0)
				{
					if(!$this->_SHELL->isOneShotCall())
					{
						...
						switch($type)
						{
							case AddonDcim\Api\Equipment::OBJECT_TYPE:
							{
								$equipments = $items;
								$items = array();
							...
							//case AddonDcim\Api\Location::OBJECT_TYPE:
							//case AddonDcim\Api\Cabinet::OBJECT_TYPE:
							default:
							{
		...
		{
			$cases = array(
				AddonDcim\Api\Location::OBJECT_TYPE => array(
					'list' => '_getLocationInfos',
					'form' => '_getLocationInfos',
				),
				AddonDcim\Api\Cabinet::OBJECT_TYPE => array(
					'list' => '_getCabinetInfos',
					'form' => '_getCabinetInfos',
				),
				AddonDcim\Api\Equipment::OBJECT_TYPE => array(
		...
		protected function _getLocationResults($location, $fromCurrentPath = true, $path = null, $recursion = false)
		{
			if($fromCurrentPath)
			{
				...
				if($currentApi instanceof AddonDcim\Api\Location) {
					$locationId = $currentApi->getLocationId();
					$locations = AddonDcim\Api\Location::searchLocations($location, $locationId, $recursion);
				}
			}
			else {
				$locations = AddonDcim\Api\Location::searchLocations($location);
			}
		...
		protected function _getCabinetResults($cabinet, $fromCurrentPath = true, $path = null, $recursion = false)
		{
			if($fromCurrentPath)
			{
				...
				if($currentApi instanceof AddonDcim\Api\Location) {
					$locationId = $currentApi->getLocationId();
					$cabinets = AddonDcim\Api\Cabinet::searchCabinets($cabinet, $locationId, $recursion);
				}
			}
			else {
				$cabinets = AddonDcim\Api\Cabinet::searchCabinets($cabinet);
			}
		...
		protected function _getEquipmentResults($equipment, $fromCurrentPath = true, $path = null, $recursion = false)
		{
			if($fromCurrentPath)
			{
				...
				if($currentApi instanceof AddonDcim\Api\Cabinet) {
					$cabinetId = $currentApi->getCabinetId();
					$equipments = AddonDcim\Api\Equipment::searchEquipments($equipment, $equipment, $equipment, $cabinetId, null);
				}
				elseif($currentApi instanceof AddonDcim\Api\Location) {
					$locationId = $currentApi->getLocationId();
					$equipments = AddonDcim\Api\Equipment::searchEquipments($equipment, $equipment, $equipment, null, $locationId, $recursion);
				}
			}
			else {
				$equipments = AddonDcim\Api\Equipment::searchEquipments($equipment, $equipment, $equipment);
			}
		...
		protected function _getLocationObjects($location, $fromCurrentPath = true, $path = null, $recursion = false)
		{
			...
			foreach($locations as &$location) {
				$location = AddonDcim\Api\Location::factory($location[AddonDcim\Api\Location::FIELD_ID]);
			}
			unset($location);
		...
		protected function _getCabinetObjects($cabinet, $fromCurrentPath = true, $path = null, $recursion = false)
		{
			...
			foreach($cabinets as &$cabinet) {
				$cabinet = AddonDcim\Api\Cabinet::factory($cabinet[AddonDcim\Api\Cabinet::FIELD_ID]);
			}
			unset($cabinet);
		...
		protected function _getEquipmentObjects($equipment, $fromCurrentPath = true, $path = null, $recursion = false)
		{
			...
			foreach($equipments as &$equipment) {
				$equipment = AddonDcim\Api\Equipment::factory($equipment[AddonDcim\Api\Equipment::FIELD_ID]);
			}
			unset($equipment);
		...
		protected function _getLocationInfos($location, $fromCurrentPath = true, $path = null, $recursion = false)
		{
			...
			foreach($locations as $location)
			{
				...
				$item['path'] = '/'.str_replace(AddonDcim\Api\Location::SEPARATOR_PATH, '/', $location['path']);
				$items[] = $item;
			}
		...
		protected function _getCabinetInfos($cabinet, $fromCurrentPath = true, $path = null, $recursion = false)
		{
			...
			foreach($cabinets as $cabinet)
			{
				...
				$item['path'] = DIRECTORY_SEPARATOR.str_replace(AddonDcim\Api\Cabinet::SEPARATOR_PATH, DIRECTORY_SEPARATOR, $cabinet['path']);
				$items[] = $item;
			}
		...
		protected function _getEquipmentInfos($equipment, $fromCurrentPath = true, $path = null, $recursion = false)
		{
			...
			foreach($equipments as $Dcim_Api_Equipment)
			{
				...
				//$item['locationPath'] = DIRECTORY_SEPARATOR.str_replace(AddonDcim\Api\Location::SEPARATOR_PATH, DIRECTORY_SEPARATOR, $Dcim_Api_Equipment->locationApi->getPath(true));
				$item['cabinetName'] = $Dcim_Api_Equipment->cabinetApi->getLabel();
				//$item['cabinetPath'] = DIRECTORY_SEPARATOR.str_replace(AddonDcim\Api\Cabinet::SEPARATOR_PATH, DIRECTORY_SEPARATOR, $Dcim_Api_Equipment->cabinetApi->getPath(true));
				$item['path'] = DIRECTORY_SEPARATOR.str_replace(AddonDcim\Api\Equipment::SEPARATOR_PATH, DIRECTORY_SEPARATOR, $Dcim_Api_Equipment->getPath());
		...
		public function printObjectInfos(array $args, $fromCurrentContext = true)
		{
			...
			{
				case AddonDcim\Api\Location::class:
					$cases = array(
					...
					break;
				case AddonDcim\Api\Cabinet::class:
					$cases = array(
		...
		protected function _getObjects($context = null, array $args = null)
		{
			...
			$items = array(
				...
				AddonDcim\Api\Cabinet::OBJECT_TYPE => array(),
				AddonDcim\Api\Equipment::OBJECT_TYPE => array(),
				AddonDcim\Api\Cable::OBJECT_TYPE => array(),
			);
			...
			/**
			  ...
			  * Voir PhpCliShell\Application\Phpipam\Shell\Program\Ipam->_getObjects
			  */
			$cases = array(
				AddonDcim\Api\Location::OBJECT_TYPE => array(
					AddonDcim\Api\Location::class => 'getSubLocationIds',
					AddonDcim\Api\Cabinet::class => 'getCabinetIds',
				),
				AddonDcim\Api\Cabinet::OBJECT_TYPE => array(false),
				AddonDcim\Api\Equipment::OBJECT_TYPE =>  array(false),
			);
				...
				foreach($cases[$currentType] as $objectClass => $objectMethod)
				{
					...
					if(C\Tools::is('array&&count>0', $objects))
					{
						...
						foreach($objects as $object)
						{
							...
							{
								case AddonDcim\Api\Location::OBJECT_TYPE: {
									$Dcim_Api_Abstract = AddonDcim\Api\Location::factory($object);
									break;
								}
								case AddonDcim\Api\Cabinet::OBJECT_TYPE: {
									$Dcim_Api_Abstract = AddonDcim\Api\Cabinet::factory($object);
									break;
								}
								default: {
					...
					elseif($currentApi instanceof AddonDcim\Api\Cabinet)
					{
						if(isset($args[1])) {
						...
						foreach($equipments as $equipment)
						{
							$Dcim_Api_Equipment = AddonDcim\Api\Equipment::factory($equipment);
							...
							$items[AddonDcim\Api\Equipment::OBJECT_TYPE][] = array(
									'name' => $objectName,
									'side' => $position['side'],
									'positionU' => $position['U'],
									'templateU' => $templateU,
					...
					elseif($currentApi instanceof AddonDcim\Api\Equipment)
					{
						$ports = $currentApi->getConnectedPortIds();

						foreach($ports as $port)
						{
							$portApi = AddonDcim\Api\EquipmentPort::factory($port);
							$nbPort = $portApi->getEndConnectedPortId();

							if($nbPort !== false)
							{
								$nbPortApi = AddonDcim\Api\EquipmentPort::factory($nbPort);

								$items[AddonDcim\Api\Cable::OBJECT_TYPE][] = array(
			...
			usort($items[AddonDcim\Api\Location::OBJECT_TYPE], $compare);
			usort($items[AddonDcim\Api\Cabinet::OBJECT_TYPE], $compare);
			usort($items[AddonDcim\Api\Cable::OBJECT_TYPE], $compare);

			/*$compare = function($a, $b)
			{
				if($a['side'] !== $b['side']) {
					return (mb_strtolower($a['side']) === AddonDcim\Api\Cabinet::SIDE_FRONT) ? (-1) : (1);
				}
			...
			usort($items[AddonDcim\Api\Equipment::OBJECT_TYPE], $compare);*/

			$items[AddonDcim\Api\Equipment::OBJECT_TYPE] = $this->_formatCabinetEquipments($currentApi, $items[AddonDcim\Api\Equipment::OBJECT_TYPE], $side, $view);

			return array(
				'location' => $items[AddonDcim\Api\Location::OBJECT_TYPE],
				'cabinet' => $items[AddonDcim\Api\Cabinet::OBJECT_TYPE],
				'equipment' => $items[AddonDcim\Api\Equipment::OBJECT_TYPE],
				'cable' => $items[AddonDcim\Api\Cable::OBJECT_TYPE]
			);
		}

		protected function _formatCabinetEquipments(AddonDcim\Api\AbstractApi $currentApi, array $equipments, $side = null, $view = null)
		{
			$results = array();
			...
			if($currentApi instanceof AddonDcim\Api\Cabinet)
			{
				...
				{
					case 'front': {
						$side = AddonDcim\Api\Cabinet::SIDE_FRONT;
						break;
					}
					case 'rear': {
						$side = AddonDcim\Api\Cabinet::SIDE_REAR;
						break;
					}
					default: {
						$side = null;
				...
				foreach($equipments as $equipment)
				{
					...
					$j = ($equipment['side'] === AddonDcim\Api\Cabinet::SIDE_FRONT) ? (0) : ($posMalus);
		...
		public function modifyEquipment(array $args)
		{
			if(count($args) >= 3)
			{
				if($args[0] === '.')
				{
					...
					if(!$Dcim_Api_Equipment instanceof AddonDcim\Api\Equipment) {
				...
				{
					$refreshPrompt = false;
					$equipments = AddonDcim\Api\Equipment::searchEquipments($args[0], null, $args[0]);
					...
					if($equipments !== false)
					{
						switch(count($equipments))
						{
							...
							case 1: {
								$equipmentId = $equipments[0][AddonDcim\Api\Equipment::FIELD_ID];
								$Dcim_Api_Equipment = AddonDcim\Api\Equipment::factory($equipmentId);
								break;
							}
							default: {
		...
		{
			return $this->_searchLastPathApi($pathApi, AddonDcim\Api\Location::class);
		}
		...
		public function shellAutoC_cd($cmd, $search = null)
		{
			...
			foreach($searchParts as $index => $search)
			{
				...
				else
				{
					...
					elseif($result instanceof AddonDcim\Api\AbstractApi)
					{
						$pathApi[] = $result;
						$results = array('');			// Workaround retourne un seul resultat avec en clé input et en valeur ''
						...
						/**
						  ...
						  */
						/*if($result instanceof AddonDcim\Api_Equipment) {
							break;
						}*/
					}
					// /!\ Ne doit jamais se réaliser!
		...
		/**
		  * @param \PhpCliShell\Addon\Dcim\Api\AbstractApi $baseApi
		...
		protected function _shellAutoC_cd_browser($baseApi, $search = null)
		{
			...
			if($baseApiClassName === AddonDcim\Api\Location::class)
			{
				$locations = $baseApi->findLocations($search.'*', false);

				if($locations !== false)
				{
					$locations = array_column($locations, AddonDcim\Api\Location::FIELD_NAME, AddonDcim\Api\Location::FIELD_ID);

					if(($locationId = array_search($search, $locations, true)) !== false) {
						$results = AddonDcim\Api\Location::factory($locationId);
					}
				...
				if($cabinets !== false)
				{
					$cabinets = array_column($cabinets, AddonDcim\Api\Cabinet::FIELD_NAME, AddonDcim\Api\Cabinet::FIELD_ID);

					if(($cabinetId = array_search($search, $cabinets, true)) !== false) {
						$results = AddonDcim\Api\Cabinet::factory($cabinetId);
					}
			...
			//if($baseApiClassName === AddonDcim\Api\Location::class || $baseApiClassName === AddonDcim\Api\Cabinet::class)
			if($baseApiClassName === AddonDcim\Api\Cabinet::class)
			{
				$equipments = $baseApi->findEquipments($search.'*', null, null, false, true);

				if($equipments !== false)
				{
					$equipments = array_column($equipments, AddonDcim\Api\Equipment::FIELD_NAME, AddonDcim\Api\Equipment::FIELD_ID);

					if(($equipmentId = array_search($search, $equipments, true)) !== false) {
						$status = true;
						$results = AddonDcim\Api\Equipment::factory($equipmentId);
					}
